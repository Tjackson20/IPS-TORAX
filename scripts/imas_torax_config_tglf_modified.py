...
def _with_initial_time(value, t_initial=T_INITIAL):
    ...
    if isinstance(value, dict):
        return {
            key: _with_initial_time(val, t_initial)
...
def _sources_from_imas(core_sources_ids):
    sources = copy.deepcopy(
        core_sources.sources_from_IMAS(core_sources_ids)
    )

    # Remove ICRH because its mapped profile is not currently used.
    sources.pop("icrh", None)

    # Remove sources listed in the environment variable.
    for source_name in DISABLED_SOURCES:
        sources.pop(source_name, None)

    # Remove empty prescribed source profiles.
    for source_name in list(sources):
        source_config = sources[source_name]
        prescribed_values = []

        for times, rho_norm, profile in source_config.get(
            "prescribed_values",
            (),
        ):
            ...
            prescribed_values.append(
                ([T_INITIAL], rho_norm, profile)
            )

        if prescribed_values:
            source_config["prescribed_values"] = tuple(
                prescribed_values
            )
        else:
    ...
    for source in core_sources_ids.source:
        ...
        elif source_name == "radiation":
            radiation_electron_loss = list(
                profile.electrons.energy
            )

        elif source_name == "synchrotron_radiation":
            synchrotron_electron_loss = list(
                profile.electrons.energy
            )

        elif source_name == "bootstrap_current":
            bootstrap_current = list(
                -1.0 * profile.j_parallel
            )

    # Add combined ion and electron heating.
    if (
        nbi_rho is not None
        and nbi_electron_heat is not None
        and nbi_ion_heat is not None
    ):
        electron_heat = list(nbi_electron_heat)

        if radiation_electron_loss is not None:
            electron_heat = [
                electron + radiation
                for electron, radiation in zip(
                    electron_heat,
                    radiation_electron_loss,
                )
            ]
        ...
        if synchrotron_electron_loss is not None:
            electron_heat = [
                ...
                for electron, synchrotron in zip(
                    electron_heat,
                    synchrotron_electron_loss,
                )
            ]
    ...
    # Add combined NBI and bootstrap current.
    if nbi_rho is not None and nbi_current is not None:
        current_profile = list(nbi_current)

        if bootstrap_current is not None:
            current_profile = [
                nbi_value + bootstrap_value
                for nbi_value, bootstrap_value in zip(
                    nbi_current,
                    bootstrap_current,
                )
            ]
...
def build_config():
    ...
    return {
        ...
        # Required top-level TORAX section.
        "pedestal": {},

        "transport": {
            "model_name": "tglf",
            "tglf_exec_path": "/home/teliyah/gacode/tglf/bin/tglf",
            "output_directory": "torax_tglf_runs_modified",

                "rho_min": 0.5,
                "rho_max": 0.95,

            # Parameters requested by Dr. Hassan.
            "sat_rule": 2,
            "find_width": False,
            "use_mhd_rule": True,
            "alpha_quench": 1.0,
...
if __name__ == "__main__":
    print("Built modified IMAS-driven TORAX TGLF config")
    ...
    print(
        ...
        sorted(CONFIG["profile_conditions"]),
    )